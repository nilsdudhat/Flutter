      ...
      builder: (context, snapshot) {
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, index) {
            final currentChatData = loadedMessages[index].data();
            final nextChatData = index + 1 < loadedMessages.length
                ? loadedMessages[index + 1]
                : null;

            final currentUserId = currentChatData["userId"];
            final nextUserId =
            ...
            if (nextUserIsSame) {
              return MessageBubble.next(
                message: currentChatData["message"],
                isMe: authenticatedUser.uid == currentUserId,
              );
            } else {
              return MessageBubble.first(
                userImage: currentChatData["userProfile"],
